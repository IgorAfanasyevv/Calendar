...
export default function Layout({
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-indigo-50/60 via-white to-rose-50/60 dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950 text-neutral-800 dark:text-neutral-100">
      {/* Кнопка бургер-меню — есть и на десктопе, и на мобильном (мобильный низ остаётся для быстрого переключения) */}
      <button
        onClick={() => setMenuOpen(true)}
        className="fixed z-40 top-4 left-4 w-11 h-11 rounded-full bg-white dark:bg-neutral-900 shadow-lg border border-neutral-200/60 dark:border-neutral-800 flex items-center justify-center text-neutral-500 hover:text-indigo-500"
        title="Меню"
      >
        <Menu size={18} />
      </button>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 flex" onClick={() => setMenuOpen(false)}>
          ...
          <aside
            className="relative w-[260px] max-w-[80vw] h-full bg-white dark:bg-neutral-900 shadow-2xl flex flex-col p-4 gap-1 animate-[modalIn_.2s_ease-out] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 px-2 py-3">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-rose-400 flex items-center justify-center shadow-md shadow-indigo-500/20 shrink-0">
                  <Heart size={18} className="text-white" fill="white" />
                </div>
                <div className="font-semibold text-sm truncate">{workspace?.name || 'Пространство'}</div>
              </div>
              <button onClick={() => setMenuOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-400 shrink-0">
                <X size={16} />
              </button>
            </div>
