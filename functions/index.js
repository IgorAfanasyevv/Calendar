...
Ровно 4 элемента в "meals" — по одному на breakfast, lunch, dinner, snack. grams — примерный вес порции в граммах (сумма граммовки ингредиентов).`;
    ...
    async function generateSingleGroup(groupLabel, proteinHint, avoidList) {
      ...
      try {
        ...
        return { ok: false, error: `Не получилось разобрать ответ модели: ${e.message}` };
      }
    }
...
${photoList.length > 1 ? `Тебе передано ${photoList.length} фото ОДНОЙ И ТОЙ ЖЕ тренировки — скорее всего это скриншоты одного длинного
списка упражнений, сделанные один за другим при прокрутке экрана вниз, поэтому у соседних фото ЧАСТО есть пересечение —
несколько упражнений внизу первого фото повторяются вверху следующего. Это ОБЯЗАТЕЛЬНО нужно учесть:
1. Сначала мысленно пройдись по каждому фото и выпиши упражнения по порядку, как они идут в списке на экране.
2. Сравни конец списка с одного фото с началом списка следующего фото — если несколько упражнений подряд совпадают по
   названию и позиции (это и есть зона перекрытия при прокрутке) — это ОДНИ И ТЕ ЖЕ упражнения, а не разные с одинаковым названием.
3. В итоговом списке каждое такое упражнение должно встретиться РОВНО ОДИН раз, объединяя все фото в единый непрерывный список тренировки.
Пример: на фото 1 внизу видно "18. Выпады, 19. Планка, 20. Скручивания", а на фото 2 вверху видно "19. Планка, 20. Скручивания, 21. Берпи" —
это значит упражнения 19 и 20 просто попали на оба фото при прокрутке, в итоговом списке они должны быть только один раз каждое,
а не дважды.` : ''}
