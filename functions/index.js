...
ВАЖНО про точность калорий и БЖУ — это самая частая причина ошибок, поэтому считай их не "на глаз", а по шагам:
Перед итоговым JSON, для КАЖДОГО блюда сначала напиши короткий расчёт обычным текстом: перечисли каждый ингредиент
с его реальной калорийностью/БЖУ на 100 г (используй свои знания о пищевой ценности продуктов — например куриная грудка
~165 ккал/100г, Б31/Ж3.6/У0; рис варёный ~130 ккал/100г, Б2.7/Ж0.3/У28), переведи граммовку каждого ингредиента в
конкретные ккал/Б/Ж/У для указанного количества (например 300 г курицы = 165×3 = 495 ккал), и просуммируй по блюду.
Только ПОСЛЕ такого расчёта для всех блюд — выведи итоговый JSON, куда впиши уже посчитанные (не выдуманные заново)
суммы. Этот расчёт — твои черновые вычисления для точности, показывать его пользователю не нужно, он не попадёт в приложение.

Когда весь расчёт закончен, выведи ИТОГОВЫЙ JSON СТРОГО в этом формате (после расчёта, без другого текста после JSON):
...
Сначала коротко (2-4 предложения) порассуждай вслух, чтобы посчитать КБЖУ максимально точно по граммам:
1. Вспомни справочную пищевую ценность ЭТОГО конкретного продукта/блюда на 100 г (используй реальные известные данные о составе, а не приблизительную "на глазок" оценку) — калории, белки, жиры, углеводы на 100 г.
